...
The Pages workflow runs on ordinary `main` pushes and as a reusable workflow called directly by a refresh job that actually committed validated data or diagnostics. The refresh job passes the exact published commit SHA, so a no-change run does not redeploy. Pages queues overlapping deployments and compares that SHA with the current `main` immediately before setup; a superseded run exits successfully without deploying an older snapshot. This direct call is required because a commit pushed with the refresh job's default `GITHUB_TOKEN` does not recursively trigger another push-based workflow. An unexpected refresh failure may still publish its already-validated diagnostics before the workflow reports failure.

The static refresh runs at `23:30 UTC` each weekday and can also be started with `workflow_dispatch`. Yahoo, FinanceDataReader, Stooq, FRED, and the ephemeral Finviz check require no provider API secret. Yahoo-family public collection nevertheless requires repository variable `YAHOO_PUBLIC_DISPLAY_APPROVED=true`. When that operating approval is absent, the collector does not call Yahoo or FinanceDataReader, records `yahoo_public_display_rights_unconfirmed`, preserves prior validated observations with an honest `degraded`/`stale` state, and continues independent FRED, KRX, or same-basis Stooq paths. An expected rights/configuration skip is a successful automation outcome; an unexpected provider or validation failure still fails after diagnostics are committed. A manual run may set `backfill=true`, optional comma-separated `asset_ids`, and optional ISO `start`/`end` dates; the scheduled path is always incremental.
...
The collector first stages a full candidate generation, validates contracts and cross-column invariants, then atomically replaces artifacts. It records the actual source/adapter and whether the independent check passed, failed, or was unavailable. Core and expanded publishers share one branch-scoped writer queue. Immediately before committing they fetch the target branch and fail closed if it advanced during collection; the next run must regenerate from that newer code and contract rather than rebasing old generated data without rerunning the collector. The workflow commits only `data/`; code and dependency changes still go through normal CI. The collector exit code is captured so validated status diagnostics can be committed before an unexpected provider failure marks the run failed. `lastSuccessAt` advances only when at least one source refreshed and no unexpected reason remains; `publication.latestPublishedAt` may still advance when a validated partial result and its failure diagnostics are intentionally published.
...
workflow records a notice and completes as a no-change skip before checkout,
dependency installation, or provider discovery when it is absent or false. It
is not an API secret. Do not set it merely because an endpoint is technically
reachable; keep using `--cache-scope local` while the rights review is
incomplete.
